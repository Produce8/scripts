# PowerShell Script to Remove Brave Extension Registry Policy
# Run this script with administrative privileges
# The browser will require a restart for changes to take effect

Write-Output "Uninstalling P8 extension policy for Brave"

# Registry path for Brave ExtensionInstallForcelist
...
if (Test-Path $regPath) {
    ...
    if ($removed) {
        ...
        Write-Output "No matching Brave extension policy found for Produce8 agent."
        exit
    }
} else {
    Write-Output "No Brave extension policy registry path found."
    exit
}

Write-Output "Brave extension policy uninstalled successfully. Restart Brave to apply changes."
